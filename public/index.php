...
$router->map('GET', '/support', 'HomeController#support', 'Support the show');
$router->map('GET', '/patreons', 'HomeController#patreons', 'List of Patreons supporting the show');
$router->map('GET', '/archive', 'HomeController#archive', 'List of podcasts');
$router->map('GET', '/podcast/[i:id]', 'HomeController#podcast', 'Listen to a podcast');

// Redirects
$router->map('GET', '/itunes', 'RedirectController#itunes', 'Redirect to podcast iTunes page');


// I have to literally make an entire function just so the user won't be confused when they type /admin instead of /admin/
// If only the creator would've put in the effort to make good documentation and give the ability to give multiple
// options for URLs
// e.g $router->map('GET', ['/admin/', '/admin'], 'RedirectController#admin', 'Redirect to admin dashboard');
$router->map('GET', '/admin', 'RedirectController#admin', 'Redirect to admin dashboard');

// Admin pages
$router->map('GET', '/admin/', 'AdminController#index', 'Admin Dashboard');
$router->map('GET', '/admin/logout', 'AdminController#logout', 'Logs out user');
$router->map('GET|POST', '/admin/login', 'AdminController#login', 'Login screen');
$router->map('GET|POST', '/admin/patreons', 'AdminController#patreons', 'Patreons screen');
$router->map('GET|POST', '/admin/upload', 'AdminController#upload', 'Upload a podcast');
